...
class HiggsStreamingVocoderScheduler:
    ...
    def _on_new_request(self, request_id: str, payload: StagePayload) -> None:
        ...
        self._stream_enabled[request_id] = streaming
        if not streaming:
    ...
    def _on_chunk(self, request_id: str, chunk: Any) -> None:
        stream_item = chunk
        metadata = getattr(stream_item, "metadata", None)
        if request_id not in self._stream_enabled and isinstance(metadata, dict):
            self._stream_enabled[request_id] = bool(metadata.get("stream"))
        state = self._stream_states.setdefault(request_id, _HiggsStreamState())

        row = getattr(stream_item, "data", stream_item)
        if not isinstance(row, torch.Tensor):
            row = torch.tensor(row, dtype=torch.long)
        row = row.detach().to(dtype=torch.long).cpu()
        if row.ndim != 1:
            raise ValueError(
    ...
    def _finalize_streaming_request(self, request_id: str) -> None:
        ...
        if output is None and not state.audio_chunks:
    ...
    def _decode_delta(
        ...
        state.next_decode_rows = delayed_count + self._stream_followup_stride
        state.audio_chunks.append(delta)
    ...
    def _vocode_payload(self, payload: StagePayload) -> StagePayload:
        ...
        if audio is None:
            ...
            return payload
        payload.data["audio_data"] = audio.to(torch.float32).cpu().numpy().tolist()
        payload.data["sample_rate"] = self._sample_rate
        payload.data["modality"] = "audio"
    ...
    def _decode_delayed_rows(
        ...
        *,
        ...
        return self._codec.decode(codes_TN).detach().to(torch.float32).cpu()

    def _build_audio_payload(self, audio: torch.Tensor) -> dict[str, Any]:
        return {
            "audio_data": audio.to(torch.float32).cpu().numpy().tolist(),
            "sample_rate": self._sample_rate,
            "modality": "audio",
        }
