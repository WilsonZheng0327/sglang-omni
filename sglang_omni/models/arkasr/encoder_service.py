...
# WhisperFeatureExtractor knobs that change the mel frontend, and so the
# encoder input, for identical audio.
...
def build_cache_namespace(
    ...
    *,
    ...
    payload = {
        ...
        "dtype": str(next(model.audio_encoder.parameters()).dtype),
        "mm_attention_backend": mm_attention_backend or "default",
        "device_type": next(model.audio_encoder.parameters()).device.type,
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._thread = threading.Thread(
            target=self._worker, name="arkasr-audio-encode", daemon=True
        )
        self._thread.start()
    ...
    def encode_item(self, item: Any) -> None:
        ...
        with self._lock:
            ...
            if future is None:
                ...
                # start work after the prior leader already cached.
                cached = self._cache.get(key)
        ...
        while True:
            ...
            try:
                ...
                if len(batch) == 1:
                    ...
                    continue
                logger.exception(
                    f"ARK-ASR batched audio encode failed for {len(items)} "
