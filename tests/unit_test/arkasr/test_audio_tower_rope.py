...
"""The cached RoPE table must be numerically identical to the per-forward
recompute it replaces, and must stay out of ``state_dict``."""
...
def test_cached_table_matches_the_original_recompute(seq_len: int) -> None:
    rope = ArkRotaryEmbedding(8, max_position=_MAX_POSITION)

    emb = rope.get_emb(seq_len, torch.float32, torch.device("cpu"))

    assert emb.shape == (seq_len, 4, 2)
    assert torch.equal(emb, _reference_emb(8, seq_len))
...
def test_repeated_calls_slice_one_table_instead_of_rebuilding() -> None:
    ...
    # Both are views of the single buffer: no arange/cos/sin on the hot path.
...
def test_table_is_not_persisted_in_state_dict() -> None:
    """load_weights matches checkpoint names; a persistent buffer would add an
    unexpected key."""
    rope = ArkRotaryEmbedding(8, max_position=_MAX_POSITION)
...
def test_low_precision_request_casts_like_the_original(dtype: torch.dtype) -> None:
    ...
    assert torch.equal(emb, _reference_emb(8, 12).to(dtype))


def test_overlong_sequence_raises_instead_of_silently_truncating() -> None:
    rope = ArkRotaryEmbedding(8, max_position=_MAX_POSITION)

    with pytest.raises(ValueError, match="max_source_positions"):
        rope.get_emb(_MAX_POSITION + 1, torch.float32, torch.device("cpu"))
...
def test_tower_forward_is_unchanged_by_the_cache() -> None:
    ...
    recomputed_table = _reference_emb(head_dim // 2, 20)
    original_get_emb = tower.rotary_embedding.get_emb
    try:
        tower.rotary_embedding.get_emb = (
            lambda seq_len, dtype, device: recomputed_table[:seq_len]
        )
        with torch.no_grad():
            reference = tower(mel)
    finally:
