...
`voices.tgz` is unpacked into `voices/` next to the checkpoint on first use (or into a temp directory if the snapshot is read-only). Recorded voice prompts (`--voice some.wav`) additionally need `pip install pyloudnorm` for the reference's loudness normalisation; the packaged `.pt` voices do not.
...
- The reply is as long as the input, offset by one frame: the model answers while it listens, so leave silence after the caller's last words if you want a full answer. Prompt plus reply must fit the engine's 8192-position context (about 10.8 minutes); a longer recording currently gets a reply cut short at that limit.
...
| `temperature`, `top_k` | Text sampling; reference defaults 0.7 / 25. `temperature=0` is greedy. |
...
`tests/unit_test/personaplex/` runs on CPU without weights: the delayed timeline, chunked Mimi against whole-sequence Mimi, depformer weight slicing and teacher forcing, and request lowering.
